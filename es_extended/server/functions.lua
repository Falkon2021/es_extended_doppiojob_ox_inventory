...
-- [[ da qui in poi funzioni aggiunte telefono]]
ESX.GetPianoTariffario = function(phone_number, cb)
    MySQL.Async.fetchAll("SELECT * FROM sim WHERE phone_number = @phone_number", {['@phone_number'] = phone_number}, function(result)
        if #result > 0 then
            cb({piano_tariffario = result[1].piano_tariffario, minuti = result[1].minuti, messaggi = result[1].messaggi, dati = result[1].dati})
        end
    end)
end

-- @param corrisponde ai 3 tipi di dato in sim, minuti, messaggi, dati
ESX.GetPianoTariffarioParam = function(phone_number, param, cb)
    MySQL.Async.fetchAll("SELECT * FROM sim WHERE phone_number = @phone_number", {['@phone_number'] = phone_number}, function(result)
        if #result > 0 then
...
-- @param corrisponde al nome di un tipo di dato
ESX.UpdatePianoTariffario = function(phone_number, param, value)
...
ESX.vectorDistance = function(v1, v2)
    distance = 0.0;
